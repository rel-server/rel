...
| `http.templates.path` | `REL_HTTP__TEMPLATES__PATH` | `/template` | Directory Jet templates are loaded from, for a route's or middleware's own `template`. See [Requests and responses](../http/requests-responses.md#rendering-html-with-a-template). |
