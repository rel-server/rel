...
// Seeds the test/dmut hotel schema with fake data — see test/README.md.
//
// A fixed seed (const below) makes every run reproducible : the volume of
// data is random-looking, but not actually different from one run to the
// next. A handful of "named" rows (marked below) are hand-picked rather than
