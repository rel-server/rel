...
# Launch the rel server against the dev database (just test-db-up first). Uses
...
test-run:
    REL_PG__URI="$(just test-db-uri)" go run ./cmd/rel
...
test-db-up:
    ...
    @echo " ready at $(just test-db-uri)"
...
test-db-seed:
    go run ./test/seed "$(just test-db-uri)"

# One-shot : tear down, bring up fresh, migrate, seed — left running afterwards
test-db-fresh: test-db-down test-db-up test-db-migrate test-db-seed
    @echo "fresh dev database ready at $(just test-db-uri)"
