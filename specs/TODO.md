...
implementing, measured against the feature list in `index.md` (the stated intent).
...
- **Introspection lifecycle** (`introspection.md`, now written up). The mechanism
  ...
  is now resolved — see the `dmut` / migrations bullet below, `migrations.md`, and the `boot`
  package.
- **Query compiler architecture.** `query-engine.md` specifies the Reading/Writing algorithms
  ...
  fixed : a row-type-taking computed column (`query-engine.md ## Scoping`'s `alias.func_name`/
...
- **Connection pool / transaction lifecycle.** Never given its own spec, but several other
  documents assume it exists : `SET LOCAL ROLE` timing (`authentication.md`), the
  commit-before-select response design (`query-engine.md ## Response Shape`), and `_data`
  ...
  `query-engine.md ## Transactions`.
  **`SET LOCAL ROLE` / auth timing — now resolved for `/rel`, one deliberate divergence
  from `authentication.md`'s literal wording** : `/rel` uses session-scoped
  `SET ROLE`/`RESET ROLE` on the pinned connection, NOT the transaction-scoped
  `SET LOCAL ROLE` `authentication.md ## Lifecycle` step 5 names unconditionally — `/rel` commits its
  ...
  Also : `rpc.md`'s "all four lifecycle steps are ordinary
  ...
  **Undocumented deployment prerequisite, found empirically (`SET ROLE` under a plain
  ...
  prerequisite for its own `authenticator`/`web_anon` pattern ; neither `query-engine.md` nor
  `authentication.md` states it yet for rel.
...
- **`dmut` / migrations** (`migrations.md`) — RESOLVED, implemented. `migrations.md` now fully
  ...
  handling. This also resolves the `introspection.md`/`SIGUSR1` reload question this
  same TODO bullet above ("Introspection lifecycle") used to reference.

## Named but empty

- **`oauth-saml.md` — 0 bytes.** `authentication.md` names the libraries
...
- **`/api/{schema}/{function}` renamed to `/rpc/{schema}/{function}`** throughout
  `rpc.md` — `/api` named nothing about the actual mechanism ; `/rpc`
...
- **`jwt.anonrole` renamed to `query.anonymous_role`**, reconciling a genuine drift :
  `authentication.md` and `query-engine.md` named what reads as the identical setting
...
- **Static file serving** (`index.md`'s "configurable file access control based on path and
  database queries" bullet) — RESOLVED, spec'd AND IMPLEMENTED. `specs/http-content.md
  ...
  final same-filesystem rename — the same limitation class as `rpc.md`'s own "no
...
- **TypeScript/JS export** (`/js/query.js`, `/js/schemas/*.ts`). Named as a feature in
  `index.md`. No spec on how types are generated from introspection + well-known
...
- **RESOLVED, remove from future passes** : `query-engine.md ### Join eligibility`'s FK-pairing
...
- `query-engine.md ## Errors` — the `RelErrorResponse` ENVELOPE shape itself is now settled and
...
- `error-handling.md ## Error Codes` was rewritten to describe only what's actually
...
- `index.md` vs. `typescript.md` disagree on where the generated TS/JS client files
  live — `index.md` says `/js/query.js`/`/js/query.ts`/`/js/schemas/schema1.js`;
  `typescript.md` says `/rel/query.js`/`/rel/query.ts`/`/rel/db.json`/`/rel/db/<schema>.json`.
...
- CLI / entrypoint structure — no section on process structure, subcommands (is `dmut` a
  ...
  **Partially resolved** : `cmd/rel/main.go` now implements the full `config` package
  (`configuration.md`, koanf-based file/env/flag merge, `$FILE$`, "no arrays",
  `ConfigReader`) and the `logging` package (`logging.md ## Configuration`/
  ...
  deliberate reading of `configuration.md ## Error handling and secrets` worth
