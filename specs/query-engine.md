...
**There is no special case anywhere in the resolver for "a node's own top-level `select`" versus "a shape-producing expression nested arbitrarily deep inside another one."** Both go through the exact same recursive mechanism (`resolveChain`'s cases for `OwnExpr`/`FullExpr`/`OwnExceptExpr`/`FullExceptExpr`/`OwnAndExpr`/`FullAndExpr`/`OwnExceptAndExpr`/`FullExceptAndExpr`/`ObjectExpr`, all delegating to the shared `buildShape` helper, `query/expression_resolve.go`). An `own_and` three levels inside an object literal builds its `Shape` and is chained into exactly the same way `own_and` used as a node's whole `select` is.

> Why one mechanism, not two : own/full_and's base-column-plus-computed-key shape is the same *kind* of thing regardless of where in the tree it appears, so a narrower special case for "top-level select" specifically has no principled justification.
...
**A direct child's own alias, embedded bare as a value nested inside another expression** (not a `.` hop — just the join alias itself, e.g. `["coalesce", "director", null]` on a `movie` query with `director` joined) resolves via `## Scoping`'s ordinary scope rule (a join alias is always in scope, same as any column) and compiles the same way (`compileChildRowValue`, `query/sql_expr.go`) : a self-contained correlated subquery, to-one only for the same reason, but selecting the child's own row alias directly — `(select <alias> from <relation> <alias> where <on-clause> and <that relation's own where>)` — rather than one column. Selecting a table alias bare like this is what makes Postgres return the row's own composite type, which a function like `coalesce()` (or anything else taking a row-typed argument) consumes directly ; a target with no matching row reads back as JSON `null`, same "zero rows -> `NULL`" rule as the column-hop case above. Selecting a child's alias as a *top-level* select entry (an ordinary embed) is a completely different path (`compileSelectField`, step 2 below) ; this is specifically the nested-as-an-operand case.

### Implementation

Per node, recursively :

1. Build the node's own select list from its `select` expression (`own`/`full` and defaulting to `full` when unspecified, `own_except`/`full_and`/etc., or explicit column/expression references), resolved against the node's relation.
...
6. A `function`-based node (a call rather than a `relation`-named table/view) is handled the same way once its result set is known (see `### Function-rooted nodes` above for how its `Relation` resolves) : this covers a set-returning function AND a single-row composite-returning one alike (`Relation != nil` either way) — both go through the ordinary `compileNode` path exactly like any relation-rooted node, behaving like any other joined relation (object vs. array per step 3), with `compileFrom` compiling the call as an ordinary FROM-clause source (Postgres allows a function call there regardless of whether it's set-returning). Only a genuinely scalar function (`Relation == nil` — a bare, non-composite return type) is the real exception : it contributes its result directly, with no `row_to_json`/`json_agg` wrapping and no `select`/`join`/`where` processing at all, since there's no relation for any of that to resolve against — this is the case referenced in `## Response Shape` ("the scalar of the result of a *genuinely* scalar function"). Gating this exception on `!ReturnsSet` alone, without also checking `Relation == nil`, is a bug, not a valid shortcut : it would also catch a single-row composite return, silently dropping its `select`/`join` and skipping `row_to_json` — producing Postgres's raw composite-literal text instead of JSON.
...
On success, rel returns the result of `row_to_json` for each row returned, or the scalar of the result of a genuinely scalar function — no `Relation` at all, a bare non-composite return type, e.g. `returns int` — as JSON. Not-set-returning alone isn't the same thing : a function returning a single composite row (`returns director`, no `setof`) still has a real `Relation` and still goes through `row_to_json` like any other node, just naturally producing one row instead of many — see `### Function-rooted nodes` and `## Reading Algorithm ### Implementation` step 6.
