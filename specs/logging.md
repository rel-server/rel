...
> Why gated on status rather than always logging every `writeError`/`writeErrorForPgErr`/`writeServerError` call : a `4xx` is client input, not a server fault — logging it again would just duplicate what the response and the access log already carry, for every malformed request a client happens to send.

`route/response.go`'s `writePlainError` itself still only ever takes a plain message string, not an `error` — it stays the low-level primitive that writes the response body. `writeServerError` is the sibling every 5xx call site with a real `error` in scope (`route/handler.go`, `route/middleware.go`, `route/upload_handler.go`, `route/requestbody.go`, `route/encode.go`) now calls instead : identical response to what `writePlainError` always wrote, plus the log line above. A `writePlainError` call with no `error` behind it at all (`errcode.NoRoleConfigured`'s "no role configured" state, an anonymous-access rejection, a template failure already logged by its own `rlog.Error(...)` call right above it) is left as-is — there's either nothing further to log, or it's already logged under its own module tag.
