...
`mimes` (plural) is each multipart part's own `Content-Type`, in the same order as `bodies` — index `i` of all three always describes the same part. `names` is each part's `Content-Disposition` FILENAME (`filename=`, what the browser called the uploaded file — usually what a file-upload route actually wants), NOT the multipart form field key (`name=`) ; falling back to the field key only for a part that has no `filename=` at all (e.g. a plain, non-file multipart text field mixed into the same submission — see below). The singular `body`/`mime` shape is for a plain (non-multipart) binary `POST` — its Content-Type is the REQUEST's own `content_type` (same value `RelHttpRequest.content_type` already carries), so `mime` here is redundant with that field, included only for symmetry with the plural form and convenience (no need to also take `req` apart just to read one field back out of it). There is no `name` parameter in the singular shape : a lone raw-binary POST isn't tied to a multipart field name, so there'd be nothing meaningful to put there.

A real multipart form frequently mixes plain (non-file) fields with file uploads in one submission (a `title` text input alongside an attached file, say) — the `names`-falls-back-to-field-key rule above means these aren't dropped : a plain field's value still lands in `bodies` (as its raw UTF-8 bytes — `convert_from(bodies[i], 'utf8')` recovers the string inside the function), `names[i]` is its field key, `mimes[i]` is typically `text/plain` (whatever `Content-Type` the client's multipart encoder chose for that part, usually unset/defaulted by the browser). Nothing is silently discarded, but there's no structural distinction in `bodies`/`names`/`mimes` between "this index is a real file" and "this index is a plain form value" beyond inspecting `mimes[i]`/whether it has a real filename — a function that cares about the difference has to make that call itself from the decoded parts, rel doesn't separate them into two different arrays.
