...
* `application/json` whose body does not actually parse as valid JSON : `400`, same as any other malformed-request case (`## Postgres Exceptions` doesn't apply — this never reaches a function at all).
...
* A route declaring `files` receiving a request whose `content_type` falls into `## Request`'s JSON or `text/*` branches (i.e. `body` would otherwise have been populated) is a `415` — there is no byte payload to hand over as a "file" separately from what's already fully represented as `body`.
